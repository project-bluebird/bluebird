...
def test_generate_arg_parser():
    ...
    assert len(args) == 1 + len(req_args) + len(
        opt_args
    ), "Expected the correct number of args"
    assert all(
        x.location == "json" for x in args
    ), "Expected all args to be in the request JSON"
    ...
    for opt in opt_args:
        ...
        assert not args[
            idx
        ].required, "Expected '{}' to be an optional argument".format(opt)
        idx += 1
...
def test_check_acid(test_flask_client):
    ...
    with app.test_request_context():
        ...
        ), "Expected invalid acid to return bad request"
        ...
        test_acid = "TST1003"
        assert test_acid not in TEST_ACIDS, (
            "test_acid should not exist in the package" "TEST_ACIDS array!"
        )

        resp = check_acid(test_acid, assert_exists=False)
        assert (
            resp is None
        ), "Expected resp to be None from a valid acid (assert_exists=False)"

        # Check that assert_exists behaves correctly for a missing aircraft
        resp = check_acid(test_acid, assert_exists=True)
        assert (
            resp.status == "404 NOT FOUND"
        ), "Expected 404 not found response for missing aircraft"
        assert resp.get_json() == "AC {} not found".format(
            test_acid
        ), "Expected appropriate message"
        ...
        assert (
            resp is None
        ), "Expected None from an existing aircraft with (assert_exists=True)"
...
def test_process_ac_cmd(test_flask_client):
    ...
    assert (
        bb_app().sim_client.last_stack_cmd == expected
