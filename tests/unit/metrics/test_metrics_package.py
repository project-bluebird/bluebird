...
def test_vertical_separation_values(test_ac_data, test_bb_provider, config_fn):
	...
	assert test_bb_provider(metric, ac1, ac2) == cfg.VERT_LOS_SCORE, \
	...
	test_ac_data.fill(TEST_DATA)
	assert test_bb_provider(metric, ac1, ac2) == expected, \
...
def test_horizontal_separation_values(test_ac_data, test_bb_provider, config_fn):
	...
	TEST_DATA['lon'][1] = 0
	test_ac_data.fill(TEST_DATA)
	assert test_bb_provider(metric, ac1,
	                        ac2) == cfg.HOR_LOS_SCORE, 'Expected the min score at 0 separation'

	TEST_DATA['lat'][1] = cfg.HOR_MIN_DIST / 60
	test_ac_data.fill(TEST_DATA)
	assert test_bb_provider(metric, ac1, ac2) == cfg.HOR_LOS_SCORE, \
		'Expected the min score at the min separation'
