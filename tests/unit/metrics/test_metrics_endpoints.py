...
def test_metric_providers(test_flask_client):
	"""
	...
	resp = test_flask_client.get(f'{API_PREFIX}/metricproviders')
	assert resp.status == '200 OK'

	expected = {'BlueBirdProvider': str(bb_settings.VERSION)}
	resp_json = resp.get_json()

	assert expected == resp_json, 'Expected to see the default BlueBird metrics and version'


def test_metric_endpoint(test_flask_client):
	"""
	...
	resp = test_flask_client.get(f'{API_PREFIX}/metric')
	assert resp.status == '400 BAD REQUEST', 'Expected no args to return 400'

	resp = test_flask_client.get(f'{API_PREFIX}/metric?name=test')
	assert resp.status == '404 NOT FOUND', 'Expected invalid metric to return 404'

	resp = test_flask_client.get(f'{API_PREFIX}/metric?name=vertical_separation')
	assert resp.status == '400 BAD REQUEST', 'Expected missing args to return 400'

	resp = test_flask_client.get(f'{API_PREFIX}/metric?name=vertical_separation&args=')
	assert resp.status == '400 BAD REQUEST', 'Expected no args to return 400'

	resp = test_flask_client.get(f'{API_PREFIX}/metric?name=vertical_separation&args=test')
