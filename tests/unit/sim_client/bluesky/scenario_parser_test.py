"""
Tests for the BlueSky scenario parser
"""

import pytest

import bluebird.sim_client.bluesky.scenario_parser as sp

_FPATH_BASE = "tests/unit/sim_client/common/"

# geoJSON sector obtained by calling geojson.dumps() on an X-shaped SectorElement.

with open(f"{_FPATH_BASE}x_sector.geojson", "r") as f:
    x_sector_geojson = f.read()

# JSON scenario obtained by calling json.dumps() on a Poisson scenario in an X-shaped
# sector generated using ScenarioGenerator
with open(f"{_FPATH_BASE}x_scenario.geojson", "r") as f:
    x_scenario_json = f.read()

# Taken from scenario_generator
...
def test_features_of_type(target):
    ...
    result = target.features_of_type(SECTOR_VALUE)
    ...
    assert sorted(result[0].keys()) == sorted(
        [NAME_KEY, LOWER_LIMIT_KEY, UPPER_LIMIT_KEY, TYPE_KEY, ROUTES_KEY]
    )
...
def test_fix_features(target):
    ...
    for fix in result:
        ...
        assert sorted(fix.keys()) == sorted(
            [LATITUDE_KEY, LONGITUDE_KEY, TYPE_KEY, NAME_KEY]
        )
...
def test_define_waypoint_lines(target):
    ...
    for res in result:
        assert res[0 : len(sp.BS_DEFWPT_PREFIX)] == sp.BS_DEFWPT_PREFIX
        expected = res[
            len(sp.BS_DEFWPT_PREFIX) : (
                len(sp.BS_DEFWPT_PREFIX) + len(sp.BS_DEFINE_WAYPOINT)
            )
        ]
        assert sp.BS_DEFINE_WAYPOINT == expected
...
def test_waypoint_properties(target):
    ...
    assert sorted(result.keys()) == sorted(
        [LATITUDE_KEY, LONGITUDE_KEY, NAME_KEY, TYPE_KEY]
    )
...
def test_route(target):
    ...
    assert sorted(result[0].keys()) == sorted(
        [
            ROUTE_ELEMENT_NAME_KEY,
            ROUTE_ELEMENT_LEVEL_KEY,
            ROUTE_ELEMENT_TYPE_KEY,
            ROUTE_ELEMENT_SPEED_KEY,
        ]
    )
    # print(result)
...
def test_create_aircraft_lines(target):
    ...
    for res in result:
        expected = res[
            len(sp.BS_DEFWPT_PREFIX) : (
                len(sp.BS_DEFWPT_PREFIX) + len(sp.BS_CREATE_AIRCRAFT)
            )
        ]
        assert sp.BS_CREATE_AIRCRAFT == expected
...
def test_add_aircraft_waypoint_lines(target):
    ...
    assert (
        len(result) == 4
    )  # Routes contain 5 waypoints, including the initial position
...
def test_add_waypoint_lines(target):
    ...
    # The test scenario contains 3 aircraft and each route contains 5 waypoints,
    # including the initial position, we expect 4 add waypoint commands per aircraft
