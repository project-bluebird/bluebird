...
class SectorElement:
    ...
    def __init__(self, name, origin, shape, lower_limit, upper_limit):
        ...
        # Note the unit kmi is "International Nautical Mile" (for full list run
        # $ proj -lu)
        proj_string = (
            f"+proj=stere +lat_0={origin[0]} +lon_0={origin[1]} +k=1 +x_0=0 "
            f"+y_0=0 +ellps={ELLIPSOID} +units=kmi +no_defs"
        )
    ...
    def __geo_interface__(self) -> dict:
        ...
        # Build the list of features: one for the boundary, one for each waypoint and
        # one for each route.
        geojson = {FEATURES_KEY: []}
        geojson[FEATURES_KEY].append(self.boundary_geojson())
        geojson[FEATURES_KEY].extend(
            [
                self.route_geojson(route_index)
                for route_index in range(0, len(self.shape.route_names))
            ]
        )
        geojson[FEATURES_KEY].extend(
            [self.waypoint_geojson(name) for name in self.shape.fixes.keys()]
        )
    ...
    def boundary_geojson(self) -> dict:
        ...
        # Fix issue with __geo_interface__ unexpectedly returning a tuple of coordinates
        # rather than a list
    ...
    def waypoint_geojson(self, name) -> dict:
        ...
        geojson = {
            ...
            GEOMETRY_KEY: self.__inv_project__(
                self.shape.fixes[name]
            ).__geo_interface__,
        }
        return geojson
    ...
    def route_geojson(self, route_index) -> dict:
        ...
        - type: "Feature"
        - geometry: a LineString feature whose 'properties' are a list of points, with
          long/lat repeating those above, plus altitudes
        - properties:
        ...
        latitudes = [
            self.__inv_project__(fix_item[1]).coords[0][1] for fix_item in route
        ]
        longitudes = [
            self.__inv_project__(fix_item[1]).coords[0][0] for fix_item in route
        ]
        geojson = {
            TYPE_KEY: FEATURE_VALUE,
            PROPERTIES_KEY: {
            ...
            GEOMETRY_KEY: self.__inv_project__(
                LineString([fix_item[1] for fix_item in route])
            ).__geo_interface__,
        }
    ...
    def fix_geometry_coordinates_tuple(self, geojson):
        ...
        # Fix issue with __geo_interface__ unexpectedly returning a tuple of coordinates
        # rather than a list.
        geojson[GEOMETRY_KEY][COORDINATES_KEY] = list(
            geojson[GEOMETRY_KEY][COORDINATES_KEY]
        )
        return geojson

    def __inv_project__(self, geom):
        """
        Helper for doing an inverse projection using the projection system specified
        above
        """
